...
Below, in section \ref{subsec:DSBR} both the advantages and limitations of 3D reconstruction via active depth sensors are discussed. It was experimentally discovered that the closer computed depth via monocular or stereo methods was to the high quality depth maps produced by active cameras, the better the camera pose registration using the FVR method. Section \ref{subsec:SCBR} briefly covers stereo input, however, since there is much research on stereo methods which often reach accuracy which approaches RGB-D sensors, more focus is given to depth sensor based input as well as monocular video input, which is the most difficult data to work with. \\

In section \ref{subsec:MVVRMethodology} the technique dubbed Monocular View Volume Reconstruction (MVVR) is discussed in terms of process and limitations. Experiments in section \ref{Sec:MonocularExperimentsSection} show that the MVVR method is capable of accurate camera translation tracking using noisy depth data generated using only monocular methods. \\
