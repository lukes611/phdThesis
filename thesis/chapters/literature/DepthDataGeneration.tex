...
In 3D reconstruction, it is often ideal to use specialized sensors which capture reliable depth data on a per pixel basic. Once such camera is the RGB-D (Red, Green, Blue \& Depth) camera. These are becoming more accurate and less expensive and will soon be found in popular mobile technologies. \\

Research by Zhang et al \cite{Zhang12Microsoft} used an RGB-D camera to generate smooth, continuously updating dense 3D reconstructions using only depth data. Here, using the data, 6 degrees of freedom were tracked. An advantage of this technique which only use depth data, is that it works in absence of visual light. Meaning it can work in the dark, unlike passive camera approaches \cite{Klein07Parallel, Newcombe10Live,Stuhmer10Real} and techniques which use color data along with depth data \cite{Henry10Rgb}. \\
...
\cite{Hirschmuller05Accurate} - proposed a semi-global matching method based on mutual information and approximation of a global smoothness constraint
