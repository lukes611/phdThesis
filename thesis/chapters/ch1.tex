...
these have become available to consumers and may soon be found in mobile technologies [cite goog]
...
early sfm algorithms: either accumulated drift (computing motion) [2] or performed loop-closure using off-line optimization 

The first monocular slam system [8] was capable of producing globally consistent maps in real time with hand-held camera used probabilistic filtering of camera and scene feature estimates
...
then systems which split tracking and mapping (global optimization) -> approach by PTAM system [17]
...
compared to filters much more features can be packed into the map[25]
...
[19] uses dense optical flow with PTAM like tracking for dense recon

this relies on camera poses coming from PTAM

[26] do the same with near real time depth map calculation
...
distance metrics have been researched using the point-plane metric [5]

[5] improves convergence rates, used for surface reconstructions which have normal data

computing closest points using ICP is expensive, there is projective data association algorithm [4]
...
[9] uses a Bayesian probability of occupancy to measure whether should be added to the grid

also can used signed distance functions [9] (SDF), it can be used to fuse partial depth scans, whilst mitigating
...
more robust to noise version of SDF -> 30

SDF can be visualized by first converting to mesh and rendering ( use marching cubes [18]) or it can be directly ray cast [21]

[23] used frame-by-frame ICP with occupancy grid, users can scan in small objects by rotating the objects with their hand
works @ 10hz, issue: does not do global optimization, cannot do large scenes . final models are optimized using [7]
sdf fusion in real time may be possible

sdf can be used for globally satisfying reconstruction

small scale reconstructions:
	[28] -> produces high quality scans using a fixed ToF sensor
	[6] demonstrate a moving handheld ToF scanner

Octomap [29] -> 3d reconstruction using occupancy grid style 

[14] uses rgb-d with kinect by frame by frame icp : feature matching is used with graph optimization for loop closure and global consistency -> global fusion presents better performance

Method by Kinect Fusion

1. measurement : 			dense vertex map and normal map pyramids are produced
2. pose estimation: 		multi-level ICP used to compute pose
3. update reconstruction:	new frames are fused into global model using a truncated signed distance function (tsdf) representation
4. rather than use loop closure: map the frame into the global model using tsdf representation
